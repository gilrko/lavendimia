...
export default class ArticlesScreen extends React.Component {
  ...
  render() {
    if (this.state.isLoading) {
      return(
      <View style={{ flex: 1 }}>
        <TopBar componentLeft="" componentRight="search" navigation={this.props.navigation} date={this.state.date} />
        ...
        <View style={{ flex: 1 }}>
          ...
          {this.state.articles.length === 0 ?
            <View style={{ flex: 1, alignItems: "center", justifyContent: "center", alignContent: "center", backgroundColor: "white" }}>
              <Text>No existen articulos</Text>
            </View>
            :
            <FlatList
              ...
              renderItem={({ item, index }) =>
                <View style={{ flex: 1 }} >
                  <TouchableHighlight style={{ flex: .8, borderColor: "#dadada", borderWidth: .3, borderRadius: 10, marginBottom: 10, marginHorizontal: 25, paddingVertical: 10, paddingHorizontal: 10, backgroundColor: "white" }} underlayColor="white" onPress={() => this.props.navigation.navigate('ArticleInfo', { article: item })}>
                    <View style={{ flexDirection: "row", padding: 5 }}>
                      <View style={{ flex: .5, justifyContent: "center" }}>
                        <Image style={{ width: 50, height: 50 }} resizeMode={"cover"} source={require('../assets/images/icons/articles1.png')} /></View>
                      <View style={{ flex: 2, flexDirection: "column", marginHorizontal: 20 }}>
                        <Text style={{ marginVertical: 2, fontSize: 16, }}>{item.id}</Text>
                        <Text style={{ color: "gray", marginVertical: 2 }}>{item.description}</Text>
                        <Text style={{ color: "gray", marginVertical: 2, justifyContent: "flex-end", alignItems: "flex-end" }}>Existencia: {item.stock}</Text>
                      </View>
                      <View style={{ flex: .5, justifyContent: "center" }}>
          ...
          <ActionButton
						buttonColor="#00AB68"
						onPress={() => this.props.navigation.navigate('ArticleInfo')}
					/>
        </View>
      )
    }
